...
const quesController = {
	...
	async askedRequestOpen (req, res) {
		try {
			askedQuestion.belongsTo(User, {foreignKey: 'userId'});
			const resData = await askedQuestion.findAll({ 
				where: {quesId: 0, quesAmt: 0, paymentSts: 0, deletedSts: 0}, attributes: ['askedQuesId', 'userId', 'quesId', 'quesAmt', 'otherQuesTitle', 'askedDate'],
				order: [
					['askedQuesId', 'desc'],
				],
				include: [{
	...
	async myQuesbyuserId (req, res) {
		try {			
			...
			askedQuestion.hasOne(CustFeedbackTbl, {foreignKey: 'askedQuesId'});
			const resData = await askedQuestion.findAll({ 
				where: {userId: userId, deletedSts: 0}, attributes: ['askedQuesId', 'userId', 'quesId', 'quesAmt', 'otherQuesTitle', 'assignServiceWorkerSts', 'workerId', 'solvedSts', 'paymentSts', 'askedDate', 'consultantSchedTime'],
				order: [
					['askedQuesId', 'desc'],
				],
				include: [
	...
	async frontendAskedQues (req, res){
		try {
			...
			const askedQuesData = await askedQuestion.create({ userId: userId, quesId: quesId, otherQuesTitle: quesTitle, quesAmt: quesAmt, askedDate: askDate, askedMonth: askMonth, askedYear: askYear });
			if(quesId>0){
				...
				res.status(200).send({status:"success",message:'request sent'});
			}else{
